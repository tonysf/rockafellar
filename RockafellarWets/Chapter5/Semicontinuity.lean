...
Definition 5.4. -/
def svOuterLimitOn (S : E → Set F) (X : Set E) (x : E) : Set F :=
  outerSetLimitAlong (nhdsWithin x X) S

/-- Formula 5(1) relative to a set `X`. -/
def svInnerLimitOn (S : E → Set F) (X : Set E) (x : E) : Set F :=
  innerSetLimitAlong (nhdsWithin x X) S

theorem svOuterLimitOn_univ (S : E → Set F) (x : E) :
    svOuterLimitOn S univ x = svOuterLimit S x := by
  rw [svOuterLimitOn, svOuterLimit, nhdsWithin_univ]

theorem svInnerLimitOn_univ (S : E → Set F) (x : E) :
    svInnerLimitOn S univ x = svInnerLimit S x := by
  rw [svInnerLimitOn, svInnerLimit, nhdsWithin_univ]
...
def SvOsc (S : E → Set F) (x : E) : Prop := svOuterLimit S x ⊆ S x

/-- **Definition 5.4**: `S` is inner semicontinuous at `x`. -/
def SvIsc (S : E → Set F) (x : E) : Prop := S x ⊆ svInnerLimit S x

/-- **Definition 5.4**: `S` is continuous at `x` when it is both outer and
inner semicontinuous there. -/
def SvContinuousAt (S : E → Set F) (x : E) : Prop := SvOsc S x ∧ SvIsc S x

/-- **Definition 5.4** relative to a set `X`. -/
def SvOscOn (S : E → Set F) (X : Set E) (x : E) : Prop :=
  svOuterLimitOn S X x ⊆ S x

/-- **Definition 5.4** relative to a set `X`. -/
def SvIscOn (S : E → Set F) (X : Set E) (x : E) : Prop :=
  S x ⊆ svInnerLimitOn S X x

/-- **Definition 5.4**: the equivalent equality form of outer
semicontinuity. -/
theorem svOsc_iff_svOuterLimit_eq {S : E → Set F} {x : E} :
    SvOsc S x ↔ svOuterLimit S x = S x :=
...
theorem SvOsc.isClosed {S : E → Set F} {x : E} (h : SvOsc S x) :
    IsClosed (S x) := by
  rw [← svOsc_iff_svOuterLimit_eq.1 h]
  exact isClosed_svOuterLimit S x

/-- For a closed-valued mapping, inner semicontinuity has the equivalent
equality form. -/
theorem svIsc_iff_svInnerLimit_eq {S : E → Set F} {x : E}
    (hclosed : SvOsc S x) :
    SvIsc S x ↔ svInnerLimit S x = S x := by
  constructor
  · intro h
...
theorem svContinuousAt_iff {S : E → Set F} {x : E} :
  ...
  constructor
  · rintro ⟨hosc, hisc⟩
    exact ⟨svOsc_iff_svOuterLimit_eq.1 hosc,
      (svIsc_iff_svInnerLimit_eq hosc).1 hisc⟩
  · rintro ⟨houter, hinner⟩
    exact ⟨svOsc_iff_svOuterLimit_eq.2 houter, hinner.symm.subset⟩
...
theorem SvIsc.eventually_mem_svDom {S : E → Set F} {x : E}
    (h : SvIsc S x) (hx : x ∈ svDom S) :
...
theorem SvIsc.mem_interior_svDom {S : E → Set F} {x : E}
    (h : SvIsc S x) (hx : x ∈ svDom S) :
    x ∈ interior (svDom S) :=
...
theorem SvIscOn.eventually_mem_svDom {S : E → Set F} {X : Set E} {x : E}
    (h : SvIscOn S X x) (hx : x ∈ svDom S) :
    ∀ᶠ y in nhdsWithin x X, y ∈ svDom S := by
  obtain ⟨u, hu⟩ := hx
  simpa [svDom] using h hu univ univ_mem

/-- The absolute notions are the case `X = univ` of the relative ones. -/
theorem svOscOn_univ {S : E → Set F} {x : E} :
    SvOscOn S univ x ↔ SvOsc S x := by
  rw [SvOscOn, SvOsc, svOuterLimitOn_univ]

theorem svIscOn_univ {S : E → Set F} {x : E} :
    SvIscOn S univ x ↔ SvIsc S x := by
  rw [SvIscOn, SvIsc, svInnerLimitOn_univ]
