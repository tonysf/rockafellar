...
6.9 from Section C.  6.9 is taken ahead of 6.7 and 6.8 deliberately: it is
self-contained, resting only on Sections A and B, whereas 6.7 is a surjective
inverse-function-theorem argument that 6.8 then specializes, and the normal
cone halves of 6.7 need the tangent-normal polarity that the book itself does
not establish until 6.21 and 6.45.
...
| 6.7 Change of coordinates | Missing | Needs the surjective form of the inverse function theorem -- the book's guide augments `F` by a basis of `ker ∇F(x̄)` to make the Jacobian square and then invokes the inverse mapping theorem -- together with a transport of tangent cones through a local diffeomorphism and a product rule for tangent cones.  The two normal cone formulas, given the tangent cone formula, are the polarity `(A⁻¹K)° = A*(K°)`, which the book does not have available until 6.21 and 6.45. |
